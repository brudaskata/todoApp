...
export class TodoListComponent implements OnInit {
  ...
  public deleteAllTodos(): void {
    this.todoservice.deleteAllTodos().subscribe(() => { this.todos = [] })
  }

  public onCheckboxChange(event: Event, checkboxId: 'notYet' | 'progress' | 'done'): void {
  ...
  public filterTodos(): void {
    const anyActiveCheckbox = this.checkboxStates.notYet || this.checkboxStates.progress || this.checkboxStates.done;
    if (!this.inSearchMode) {
      if (!anyActiveCheckbox) {
        ...
        this.filteredTodos = this.todosReceived.filter(todo => {
          return (this.checkboxStates.notYet && todo.status === 'not started yet') ||
            (this.checkboxStates.progress && todo.status === 'in progress') ||
  ...
  public searchTodo(): void {
    ...
    if (!this.inFilterMode) {                                       //keresés kereső módban
      this.searchedTodos = this.todosReceived.filter(todo =>
    ...
    } else {                                                      //keresés szűrt módban a szűrt listán
      this.todos = this.filteredTodos.filter(todo =>
        todo.title.toLocaleLowerCase().includes(this.searchTerm) ||
        todo.description.toLocaleLowerCase().includes(this.searchTerm));
    }
